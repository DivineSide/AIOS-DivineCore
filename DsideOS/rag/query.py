...
# Retrieval tracing (RETRIEVER-kind spans) — no dependency on worker/ (rag/
# is a standalone module, also run as a CLI outside the worker process), and
# no dependency on tracing being configured at all: opentelemetry.trace.
# get_tracer() always returns a usable no-op tracer if no provider was ever
# registered (worker/tracing.py registers one; standalone `python query.py`
...
# EMBED_PROVIDER switches which embedding model/table set is used for
# retrieval — "openai" (default, unchanged, 1536-dim, book_passages/pyq_chunks)
# or "e5" (intfloat/multilingual-e5-base, local CPU inference, 768-dim,
# book_passages_e5/pyq_chunks_e5 — see migrations/006_e5_parallel_tables.sql).
# Evaluated once at import: switching requires a process restart, same as
# every other env-flagged behaviour in this file (HYBRID, HYDE below).
EMBED_PROVIDER = os.environ.get("EMBED_PROVIDER", "openai")
E5_MODEL_NAME  = "intfloat/multilingual-e5-base"

_PASSAGES_TABLE  = "book_passages_e5" if EMBED_PROVIDER == "e5" else "book_passages"
_PYQ_TABLE       = "pyq_chunks_e5" if EMBED_PROVIDER == "e5" else "pyq_chunks"
...
def _embed(text: str) -> list[float]:
    """Query-side embedding. e5-base requires a "query: " prefix (per its model
    card — it was trained with query/passage prefixes baked in as distinct
    encoding contexts; corpus text was re-embedded with "passage: ", see
    reembed_e5.py). Omitting the prefix on either side measurably hurts
    retrieval quality — this is not cosmetic."""
    if EMBED_PROVIDER == "e5":
        emb = _e5().encode("query: " + text, normalize_embeddings=True, show_progress_bar=False)
        return emb.tolist()
...
async def pyq_rag_lookup(
    ...
    yet — fail soft)."""
    with _tracer.start_as_current_span("pyq_rag_lookup") as span:
        span.set_attribute("openinference.span.kind", "RETRIEVER")
        span.set_attribute("input.value", topic)
        span.set_attribute("retrieval.subject", subject)
        span.set_attribute("retrieval.format", format or "")
        span.set_attribute("retrieval.exam", exam or "")
        try:
            embedding = await asyncio.to_thread(_embed, topic)
            results = await asyncio.to_thread(_pyq_search, embedding, subject, top_k,
                                              threshold, format, exam)
            for i, doc in enumerate(results):
                span.set_attribute(f"retrieval.documents.{i}.document.content", doc.get("text", "")[:2000])
                span.set_attribute(f"retrieval.documents.{i}.document.score", doc.get("similarity", 0.0))
            span.set_attribute("output.value", f"{len(results)} pyq examples")
            return results
...
def _hybrid_search(embedding: list[float], query_text: str, top_k: int,
    ...
    dense_sql = f"""
        ...
        FROM {_PASSAGES_TABLE} {subj_where}
...
async def passage_lookup(
    ...
    with _tracer.start_as_current_span("passage_lookup") as span:
        span.set_attribute("openinference.span.kind", "RETRIEVER")
        span.set_attribute("input.value", topic)
        span.set_attribute("retrieval.subject", subject or "")
        span.set_attribute("retrieval.top_k", top_k)
        span.set_attribute("retrieval.mode", "hybrid" if HYBRID else "dense")
        ...
        try:
            ...
            for i, doc in enumerate(results):
                span.set_attribute(f"retrieval.documents.{i}.document.id", doc.get("book", ""))
                span.set_attribute(f"retrieval.documents.{i}.document.content", doc.get("text", "")[:2000])
                span.set_attribute(f"retrieval.documents.{i}.document.score", doc.get("similarity", 0.0))
            span.set_attribute("output.value", f"{len(results)} passages")
            return results
        except Exception as e:
            span.record_exception(e)
            return []
