...
# resources/ and templates/ are client-specific — see build_answer_key.py's
# CLIENT_DIR comment.
CLIENT_DIR = BASE.parent / "clients" / "target-academy"
FRONT_PAGE_DOCX = CLIENT_DIR / "resources" / "front-page.docx"  # made by extract_front_page.py
...
if __name__ == "__main__":
    ...
    q = Path(sys.argv[1]) if len(sys.argv) > 1 else CLIENT_DIR / "review" / "spike3-questions.json"
    if len(sys.argv) > 2:
        out = Path(sys.argv[2])
    else:
        # client-facing filename from the questions JSON ("filename" key)
        data = json.loads(q.read_text(encoding="utf-8"))
        out = CLIENT_DIR / "review" / data.get("filename", "Practice Paper - Target Academy.docx")
    build(q, out)
    verify(out)
