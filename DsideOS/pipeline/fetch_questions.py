...
# Corpus files loaded once for AI generation — corpus/ is generic, lives
# under DsideOS/ itself (BASE, unchanged by the client-folder split below).
...
def main() -> None:
    ...
    out_path = Path(args.out) if args.out else (
        CLIENT_DIR / "review" / f"{re.sub(r'[^\\w]', '-', args.topic)[:40]}.json"
    )
