"""
One-shot script to fetch a Gmail refresh token. Run once, paste result into .env.

Usage (from divinecore-v2/):
    docker compose run --rm worker python -m sales_os.integrations.instantly.gmail_oauth_bootstrap

Requires GMAIL_OAUTH_CLIENT_ID + GMAIL_OAUTH_CLIENT_SECRET in .env first.

Scopes requested:
  - gmail.compose  -> create drafts in your inbox
  - gmail.readonly -> find the existing thread for a reply so the draft threads correctly

These are intentionally narrower than gmail.modify (which also permits
deletion/labels). If you later need labels or send-from-bot capability,
broaden the scope and re-run this bootstrap.
...
Flow:
  ...
  4. Script prints the refresh token. Paste it into .env as GMAIL_OAUTH_REFRESH_TOKEN.
"""
