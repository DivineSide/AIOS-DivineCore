...
ASSETS = BASE / "templates" / "poster_assets"
...
if __name__ == "__main__":
    ...
    data = Path(sys.argv[1]) if len(sys.argv) > 1 else BASE / "review" / "poster-demo.json"
    content = json.loads(data.read_text(encoding="utf-8"))
    out = Path(sys.argv[2]) if len(sys.argv) > 2 else BASE / "review" / content.get(
