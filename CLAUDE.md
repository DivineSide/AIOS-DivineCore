...
- **Never poll a backgrounded task with a blocking wait in a loop.** The harness sends an automatic completion notification the moment a `run_in_background` command finishes — that notification IS the trigger. Repeatedly calling a blocking-wait tool back-to-back on the same task just burns tokens re-asking a question that will be answered unprompted; it does not make the task finish faster. If you need to sanity-check that a long job hasn't silently died, check ONCE via a cheap direct signal (process CPU time trending up, a row-count/checkpoint-count query) and then stop — don't re-poll on a timer. Let the notification do its job.
